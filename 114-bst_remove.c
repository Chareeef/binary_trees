...
/**
 * inorder_succ - Returns the minimum value of a binary search tree.
...
/**
 * bst_remove_node - Removes a node from a binary search tree recursively.
 ...
 */
bst_t *bst_remove_node(bst_t *root, bst_t *node, int value)
{
	if (node != NULL)
	{
		if (node->n == value)
		...
		if (node->n > value)
			return (bst_remove_node(root, node->left, value));
		return (bst_remove_node(root, node->right, value));
	}
	return (NULL);
}
 ...
 * @root: root of the BST (pointer)
 * @value: value of the node to be deleted
 *
 * Return: new rooot node
 */
bst_t *bst_remove(bst_t *root, int value)
{
	return (bst_remove_node(root, root, value));
}
